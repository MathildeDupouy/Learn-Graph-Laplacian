...
class Graph():
    ...
    def values(self):
        ...
        return [v for (k,v) in nx.get_node_attributes(self.nx_graph,'value').items()]
    ...
    def add_one_attribute(self,node,attr,attr_name='value'):
        print(node, attr)
        self.nx_graph.add_node(node,value=attr)
    ...
    def get_attr(self,vertex):
        return self.nx_graph.nodes[vertex]
    ...
    def distance_matrix(self,method='shortest_path',changeInf=True,maxvaluemulti=10,force_recompute=False): 
        ...
        if (self.C is None) or force_recompute:
            ...
            if method=='weighted_shortest_path':
                d=self.reshaper(np.array([v for (k,v) in nx.get_node_attributes(self.nx_graph,'value').items()]))
                D= ot.dist(d,d)
...
def wl_labeling(graph,h=2,tohash=True):
    ...
    l_aux = list(nx.get_node_attributes(graph_relabel,'value').values())
    ...
    while niter<=h:
        ...
        l_aux = list(nx.get_node_attributes(graph_relabel,'value'+str(niter-1)).values())
        ...
        nx.set_node_attributes(labeled_graph,dict_,'value'+str(niter))
        niter+=1
    ...
    nx.set_node_attributes(graph2,dict_values,'value')
...
def graph_colors(nx_graph,vmin=0,vmax=7):
    ...
    cpick.set_array([])
    val_map = {}
    for k,v in nx.get_node_attributes(nx_graph,'value').items():
...
def draw_rel(G,draw=True,shiftx=0,shifty=0,return_pos=False,with_labels=True,swipy=False,swipx=False,vmin=0,vmax=7):
    ...
    if with_labels:
        nx.draw(G,pos,with_labels=True,labels=nx.get_node_attributes(G,'value'),node_color = colors)
    else:
